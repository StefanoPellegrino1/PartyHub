...
const Lotes = () => {
  ...
  const handleSubmit = async (e) =>{
      ...
      const agregarLote = async () =>{
        ...
        if(lote.length === 0 || !loteHabilitados){
                    const {data, error} = await supabase
          .from('Lote')
          .insert([{nombreLote, precioEntrada, cantidadEntradas, codigo_fiesta, estado_lote: "Habilitado"}])
          ...
          if(data){
            ...
            const {data, error} = await supabase
          .from('Lote')
          .insert([{nombreLote, precioEntrada, cantidadEntradas, codigo_fiesta, estado_lote: "Deshabilitado"}])
  ...
  return (
    <div className='px-5 mt-3'>
      ...
      <button data-bs-toggle="modal" data-bs-target="#exampleModal" className='btn btn-success'>
        Agregar Lote
      </button>
      ...
      <div class="modal-body">
        ...
        <div className="modal-caja">
          <div className="modal-datos">
            ...
            <input onChange={(e) => setNombreLote(e.target.value)} className='input-modal' type="text" />
          </div>
          <div className="modal-datos">
          <span className='span-moda'>Cantidad Entradas</span>
              <input onChange={(e) => setCantidadEntradas(e.target.value)}  className='input-modal' type="number" min="1"/>
          </div>
          <div className="modal-datos">
          <span className='span-moda'>Precio Entrada</span>
              <input onChange={(e) => setPrecioEntrada(e.target.value)} className='input-modal' type="number" min="1"/>
          </div>
        </div>
      </div>
                          ...
                          <tr key ={index}>
                             <td>{index +1}</td>
                              ...
                              <td>{e.estado_lote}</td>
                               ...
                               <td>
                                  <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#editLoteModal" className='btn btn-info btn-sm me-2'>Edit</button>
